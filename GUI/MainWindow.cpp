    ...
    QMainWindow(parent), ui(new Ui::Main)
{
    ...
    QObject::connect(ui->actionCell_Shading, SIGNAL(triggered()), builder, SLOT(activaCel_Shading()));
...
void MainWindow::on_valBoundingVolumes_stateChanged()
{
    ...
    if (ui->valBoundingVolumes->isChecked()) {
        builder->showBoundingVolumes(true);
    } else {
        builder->showBoundingVolumes(false);
    }
}
...
void MainWindow::refreshWindow() {
    ...
    ui->valSamples->setValue(visSetUp->getSamples());
